      ...
      <PopoverTrigger as-child>
        ...
        >
          <template v-if="isImage">
            ...
            />
            <div
              class="absolute inset-x-0 top-0 flex items-start justify-between gap-2 px-2 pt-1.5 pb-5 bg-gradient-to-b from-black/75 via-black/40 to-transparent transition-opacity pointer-events-none can-hover:opacity-0 can-hover:group-hover:opacity-100 focus-within:opacity-100"
            >
          ...
          <DownloadLink
            v-if="!isImage"
            :url="attachment.url"
            class="absolute top-1.5 right-1.5 transition-opacity can-hover:opacity-0 can-hover:group-hover:opacity-100 focus-visible:opacity-100"
          />
        </div>
      </PopoverTrigger>
